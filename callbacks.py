...
def register_callbacks(app):
    ...
    @app.callback(
        [Output("file-browser-modal", "is_open"),
         Output("file-browser-current-path", "value"),
        ...
        [Input("browse-ov-bin-path", "n_clicks"),
         ...
         Input({"type": "browse-button", "target": ALL}, "n_clicks"),
         Input("file-browser-cancel", "n_clicks"),
         Input("file-browser-select", "n_clicks"),
         Input("file-browser-go-up", "n_clicks"),
         Input({"type": "file-browser-item", "path": ALL}, "n_clicks")],
        [State("file-browser-modal", "is_open"),
         State("file-browser-current-path", "value"),
    ...
    def handle_file_browser(ov_bin_clicks, model_xml_clicks, browse_buttons_clicks, 
                           cancel_clicks, select_clicks, go_up_clicks, item_clicks,
                           is_open, current_path, target, mode, selected_file, ov_bin_path, model_xml_path, model_inputs,
                           inference_settings_is_open):
        ctx = callback_context
        if not ctx.triggered:
            return is_open, current_path, target, [], mode, selected_file

        # Get the trigger_id first
        trigger_id = ctx.triggered[0]["prop_id"].split(".")[0]

        # Don't open the file browser if the inference settings modal is open
        # unless a browse button is explicitly clicked by the user
        if inference_settings_is_open and not is_open:
            # Only allow explicit browse button clicks from the actual buttons
            # Check if any of the browse buttons were actually clicked (n_clicks > 0)
            browse_button_clicked = False

            # Check if the standard browse buttons were clicked
            if ov_bin_clicks is not None and ov_bin_clicks > 0 and trigger_id == "browse-ov-bin-path":
                browse_button_clicked = True
            elif model_xml_clicks is not None and model_xml_clicks > 0 and trigger_id == "browse-model-xml-path":
                browse_button_clicked = True
            # Check if any of the pattern-matching browse buttons were clicked
            elif browse_buttons_clicks is not None and any(clicks is not None and clicks > 0 for clicks in browse_buttons_clicks):
                # Only consider it a click if the trigger_id matches a browse button pattern
                if trigger_id.startswith("{") and (
                    '"type":"browse-button"' in trigger_id or 
                    '"target":"model-input-' in trigger_id
                ):
                    browse_button_clicked = True

            # If no browse button was actually clicked, prevent the file browser from opening
            if not browse_button_clicked:
                return False, current_path, target, [], mode, selected_file

        # Helper function to handle model input browse buttons
        def get_model_input_path(input_name):
            """Find the corresponding input value and determine the appropriate directory to open."""
            input_value = ""

            # First try to find the input value in the model_inputs list
            try:
                # Access states_list safely
                states = ctx.states_list[5]  # This should be the model_inputs state
                if isinstance(states, list) and len(states) > 0 and isinstance(states[0], dict) and "id" in states[0]:
                    for i, name in enumerate(states[0]["id"]):
                        if isinstance(name, dict) and "name" in name and name["name"] == input_name:
                            if i < len(model_inputs):
                                input_value = model_inputs[i]
                            break
            except (IndexError, TypeError, KeyError):
                pass

            # If we couldn't find the input value in the model_inputs list, try to find it directly
            if not input_value and model_inputs:
                # Try to find an input with the matching name
                for inp in model_inputs:
                    if inp and isinstance(inp, str):
                        input_value = inp
                        break

            # If input_value is a file path, use its directory
            # If it's a directory path, use it directly
            # If it's empty or not a valid path, use current working directory
            if input_value:
                # Always try to use the directory of the input value first
                parent_dir = os.path.dirname(input_value)
                if parent_dir and os.path.isdir(parent_dir):
                    return parent_dir
                elif os.path.isdir(input_value):
                    return input_value
                else:
                    # Try to find a valid directory in the input value path
                    path_parts = input_value.split(os.sep)
                    for i in range(len(path_parts), 0, -1):
                        test_path = os.sep.join(path_parts[:i])
                        if test_path and os.path.isdir(test_path):
                            return test_path

            # Default to current working directory if no valid path found
            return os.getcwd()

        # Handle opening the browser from different buttons
        if trigger_id == "browse-ov-bin-path":
            path = ov_bin_path if ov_bin_path else os.getcwd()
            # For OpenVINO bin path, we want to select a directory
            return True, path, "ov-bin-path", create_file_browser_content(path), "directory", ""

        elif trigger_id == "browse-model-xml-path":
            path = os.path.dirname(model_xml_path) if model_xml_path else os.getcwd()
            # For model XML path, we want to select a file
            return True, path, "model-xml-path", create_file_browser_content(path, selected_file), "file", selected_file

        elif trigger_id.startswith("{"):
            # Handle pattern-matching callbacks
            parsed_trigger = None

            # Try to parse the trigger_id
            try:
                # Use ast.literal_eval which is safer for parsing Python literals
                import ast
                parsed_trigger = ast.literal_eval(trigger_id)
            except (SyntaxError, ValueError):
                try:
                    # Fallback to the original method if literal_eval fails
                    parsed_trigger = json.loads(trigger_id.replace("'", '"'))
                except json.JSONDecodeError:
                    # If both parsing methods fail, try to extract information from the partial string
                    # Check if it's a model input button with a partial string
                    if '"target":"model-input-' in trigger_id or trigger_id == '{"target":"model-input-input':
                        try:
                            # Extract the input name from the partial string
                            if trigger_id == '{"target":"model-input-input':
                                input_name = "input"
                            else:
                                try:
                                    input_part = trigger_id.split('"target":"model-input-')[1]
                                    # Handle the case where the string is truncated after "model-input-input"
                                    if input_part == "input" or input_part.startswith("input"):
                                        input_name = "input"
                                    else:
                                        input_name = input_part.split('"')[0] if '"' in input_part else input_part
                                except (IndexError, ValueError):
                                    input_name = "input"  # Default to "input" if extraction fails

                            # Get the path for this input
                            path = get_model_input_path(input_name)
                            # For model inputs, we want to select a file
                            return True, path, f"model-input-{input_name}", create_file_browser_content(path, selected_file), "file", selected_file
                        except Exception:
                            pass

                    # Default fallback if we couldn't parse the trigger_id
                    return is_open, current_path, target, create_file_browser_content(current_path, selected_file) if is_open else [], mode, selected_file

            # If we successfully parsed the trigger_id
            if parsed_trigger:
                if parsed_trigger.get("type") == "browse-button":
                    target_id = parsed_trigger.get("target")
                    if target_id and target_id.startswith("model-input-"):
                        input_name = target_id.replace("model-input-", "")
                        path = get_model_input_path(input_name)
                        # For model inputs, we want to select a file
                        return True, path, target_id, create_file_browser_content(path, selected_file), "file", selected_file

                elif parsed_trigger.get("type") == "file-browser-item":
                    path = parsed_trigger.get("path")
                    if path and os.path.isdir(path):
                        # If it's a directory, navigate to it
                        return True, path, target, create_file_browser_content(path, selected_file), mode, selected_file
                    # If it's a file, handle based on mode
                    if mode == "file" and path and os.path.isfile(path):
                        # In file mode, update the selected file path
                        return True, current_path, target, create_file_browser_content(current_path, path), mode, path
                    # In directory mode or if path is not a file, do nothing
                    return is_open, current_path, target, create_file_browser_content(current_path, selected_file), mode, selected_file

            # If we get here, we couldn't handle the trigger_id
            return is_open, current_path, target, create_file_browser_content(current_path, selected_file) if is_open else [], mode, selected_file

        # Handle navigation and selection
        elif trigger_id == "file-browser-go-up":
            parent_path = os.path.dirname(current_path)
            return True, parent_path, target, create_file_browser_content(parent_path, selected_file), mode, selected_file

        elif trigger_id == "file-browser-select":
            return False, "", "", [], mode, ""

        elif trigger_id == "file-browser-cancel":
            return False, "", "", [], mode, ""
    ...
    def update_path_from_browser(select_clicks, current_path, target, mode, selected_file, ov_bin_path, model_xml_path, model_inputs):
        if not select_clicks or not current_path or not target:
            return ov_bin_path, model_xml_path, model_inputs

        # Update the appropriate input field
        if target == "ov-bin-path":
            # For OpenVINO bin path, always use directory mode
            return current_path, model_xml_path, model_inputs
        elif target == "model-xml-path":
            # For model XML path, use file mode if a file is selected
            if mode == "file" and selected_file and os.path.isfile(selected_file):
                return ov_bin_path, selected_file, model_inputs
            else:
                return ov_bin_path, current_path, model_inputs
        elif target.startswith("model-input-"):
            input_name = target.replace("model-input-", "")
            updated_inputs = list(model_inputs)

            try:
                # Access states_list safely
                states = callback_context.states_list[6]  # This should be the model_inputs state (index updated due to new states)
                if isinstance(states, list) and len(states) > 0 and isinstance(states[0], dict) and "id" in states[0]:
                    for i, name in enumerate(states[0]["id"]):
                        if isinstance(name, dict) and "name" in name and name["name"] == input_name:
                            # For model inputs, use file mode if a file is selected
                            if mode == "file" and selected_file and os.path.isfile(selected_file):
                                updated_inputs[i] = selected_file
                            else:
                                updated_inputs[i] = current_path
                            break
            except (IndexError, TypeError, KeyError):
                pass

            return ov_bin_path, model_xml_path, updated_inputs

        # Default case - no changes
        return ov_bin_path, model_xml_path, model_inputs

    def create_file_browser_content(path, selected_file=""):
        try:
            items = []

            # Add folders first
            for item in sorted([d for d in os.listdir(path) if os.path.isdir(os.path.join(path, d))]):
                full_path = os.path.join(path, item)
                items.append(
                    html.Div(
                        dbc.Button(
                            f"📁 {item}/",
                            id={"type": "file-browser-item", "path": full_path},
                            color="link",
            ...
            # Then add files
            for item in sorted([f for f in os.listdir(path) if os.path.isfile(os.path.join(path, f))]):
                full_path = os.path.join(path, item)
                # Highlight the selected file
                button_color = "primary" if full_path == selected_file else "link"
                items.append(
                    html.Div(
                        dbc.Button(
                            f"📄 {item}",
            ...
            if not items:
                return [html.Div("No files or folders found in this directory.", className="text-muted p-3")]

            return items
        except Exception as e:
            return [html.Div(f"Error accessing directory: {str(e)}", className="text-danger p-3")]

<script lang="ts">
	...
	// Signed difference: main - ref
	let signedDiff = $derived.by(() => {
	...
	let globalRange = $derived.by((): [number, number] | undefined => {
		...
		const stats = computeStats(signedDiff);
		if (symmetric) {
	...
	let offscreenImage = $derived.by(() => {
		...
		return valueToImageData(sliceData.data, sliceData.w, sliceData.h, colormap, sliceRange);
	});

<script lang="ts">
	...
	let gradDiff = $derived.by(() => {
		...
		for (let i = 0; i < out.length; i++) out[i] = Math.abs(mainGrad[i] - refGrad[i]);
		return out;
	});

	let displayData = $derived(showMode === 'ref' ? refGrad : showMode === 'main' ? mainGrad : gradDiff);

	let range = $derived.by((): [number, number] | undefined => {
		if (!sharedRange) return undefined;
		let lo = Infinity, hi = -Infinity;
		for (const arr of [refGrad, mainGrad]) {
	...
	let displayRange = $derived(showMode === 'diff' ? undefined : range);

	let offscreenImage = $derived.by(() => {
		return valueToImageData(displayData, refSlice.w, refSlice.h, colormap, displayRange);
	});

	let baseScale = $derived.by(() => {
		if (!canvas || !refSlice) return 1;
	...
	let displayStats = $derived(computeStats(displayData));
	...
	function redraw() {
		...
		const r = displayRange || [stats.min, stats.max];
		drawColorbar(ctx, 10, dh - 30, Math.min(200, dw - 20), 12, colormap, r[0], r[1]);
	}
...
<div class="flex flex-col gap-4 relative h-full">
	<div class="flex flex-wrap gap-4 items-center text-xs">
		...
		<label class="flex items-center gap-2">
			...
			<select use:rangeScroll bind:value={showMode} class="bg-surface-base border border-edge rounded px-1.5 py-0.5 text-xs text-gray-300">
				<option value="diff">Edge Difference</option>
				<option value="ref">{refLabel} Edges</option>
				<option value="main">{mainLabel} Edges</option>
			</select>
		</label>
		<label class="flex items-center gap-2">
	...
	{#if showTooltip && hoverX >= 0 && hoverY >= 0 && refSlice}
		...
		>
			...
			<div><span class="text-blue-400">{refLabel} grad:</span> {formatValue(refGrad[idx])}</div>
			<div><span class="text-red-400">{mainLabel} grad:</span> {formatValue(mainGrad[idx])}</div>
			<div><span class="text-gray-400">|Grad diff|:</span> {formatValue(gradDiff[idx])}</div>
		</div>
	{/if}
</div>

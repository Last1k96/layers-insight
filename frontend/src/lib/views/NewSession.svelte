    ...
    <form class="space-y-3" onsubmit={(e) => { e.preventDefault(); handleSubmit(); }}>
      ...
      <div class="grid grid-cols-2 gap-3">
        <div>
          ...
          <select id="main-device" bind:value={mainDevice} class="w-full px-2 py-1.5 bg-[--bg-input] border rounded text-sm focus:border-accent focus:outline-none {changedFields.has('main_device') ? 'border-yellow-500' : 'border-[--border-color]'}">
